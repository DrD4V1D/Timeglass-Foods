...
def main(argv: list[str] | None = None) -> int:
   ...
   This function intentionally contains no domain logic.
    ...
    direct_map = build_direct_map(recipes)
    if args.verbose:
...
def parse_json_lenient(raw: str) -> Any:
    ...
    What this does:
    - Strips whitespace and UTF-8 BOM if present
    - Parses the first valid JSON value using JSONDecoder.raw_decode()
    - Ignores any trailing non-JSON content (common "extra data" issue)
    What this does NOT do:
    - It does not support comments (// or /* */)
    - It does not fix trailing commas
    - It does not attempt JSON5
    This is intentionally conservative: it avoids "fixing" malformed JSON in ways
    that could silently change meaning.
    """
    if not isinstance(raw, str):
        raise TypeError(f"raw must be a str, got {type(raw)!r}")
    # Normalize and strip UTF-8 BOM if present
    s = raw.lstrip("\ufeff").strip()
    if not s:
    ...
    try:
        ...
        # Provide a slightly nicer error message with context
        start = max(0, e.pos - 80)
        end = min(len(s), e.pos + 80)
        snippet = s[start:end].replace("\n", "\\n")
        raise ValueError(
            f"Failed to parse JSON (lenient). "
            f"Error at pos {e.pos}: {e.msg}. Context: ...{snippet}..."
        ) from e
    # We intentionally ignore trailing content.
    return obj
# ------------------------------------------------------------------------------
# Recipe discovery
# ------------------------------------------------------------------------------
            ...
            if isinstance(v, (dict, list, str)):
                ...
                if isinstance(v, dict) and ("item" in v or "id" in v or "name" in v):
                    ...
                    if any(isinstance(x, dict) \
                      and ("item" in x or "id" in x or "name" in x) for x in v):
                        out |= from_result_obj(v)
        return out
    outputs: Set[str] = set()
    ...
    *,
    ...
    for path in sorted(nodes_dir.glob("*.json")):
        try:
            ...
            # Corrupt or unreadable node file; skip indexing it
            continue
        node_id = obj.get("id")
